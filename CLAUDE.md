...
| `docs/status.yaml`                   | Per-module and per-source file counts, as data — every number derived from the trees, so drift is detectable rather than silent. Prefer it over the tables in the doc below. Regenerated by a local script (`scripts/`, not shipped) |
| `docs/IMPLEMENTATION_STATUS.md`      | Progress tracking — remaining gaps, priorities, and the narrative behind them. Never state a global % from it       |
