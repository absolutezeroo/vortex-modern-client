...
- ✅ **Localization hashes current asset compatibility**: `external_flash_texts`-only hash files now populate the AS3 `external_texts` slot without a fallback fetch, while `GameDataResources.isValid()` keeps the win63 required resource set (`external_texts`, `external_variables`, `furnidata`, `productdata`).
...
- ✅ **Configuration/localization .txt AS3 parity pass**: client `common_configuration`/`localization_configuration` text assets are now parsed before external configuration download, `HabboProperty.EXTERNAL_VARIABLES` is restored to `external.variables.txt`, hash parsing accepts AS3 `furnidata`/`productdata` plus current `external_renderer_variables`/`external_ui_variables`/`external_flash_texts`, and localization loads `.txt` `key=value` flash texts through the AS3 parser instead of exposing raw `{key}={value}` content.
