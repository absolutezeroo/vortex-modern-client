...
### Recently completed
- ✅ **Avatar partsets AS3 XML parity fix**: `avatar.partsets.url` now reads the response as text, rejects HTML fallback pages with an explicit partsets error, accepts AS3 XML `HabboAvatarPartSets`, and preserves JSON-converted partset support through `PartSetsData.parse()`.
