...
export class BadgesMessageParser implements IMessageParser
{
    ...
    /**
     * This fragment's badges, decoded.
     *
     * DEVIATION: AS3's `currentFragment` hands back the raw per-fragment buffer (`_SafeCls_481`,
     *   a keyed collection) and leaves the field decoding to `BadgesModel.initBadges()`, after
     *   `HabboInventory.onBadges()` has concatenated every fragment. This port decodes inside
     *   `parse()` and hands back the decoded array instead, so what
     *   `HabboInventory.addBadgeMessageFragment()` concatenates is arrays rather than bytes.
     *   Same ordering, same completeness rule, one less pass over the data — the reassembly
     *   itself is ported and handles `totalFragments > 1`.
     */
    // AS3: sources/WIN63-202607011411-782849652/src/unknowns/_SafePkg_3206/_SafeCls_3564.as::get currentFragment()
