...
import {NewFriendEntityTab} from './NewFriendEntityTab';
import {Tab} from './Tab';

const log = Logger.getLogger('habbo.friendbar.tabs.NewFriendRequestTab');

/**
 * NewFriendRequestTab
 *
 * The new-UI single-request slot. Byte for byte the same class as `FriendRequestTab`
 * in AS3 — same layout, same colours, same handlers — differing only in what it
 * extends: `NewFriendEntityTab` rather than `FriendEntityTab`. Kept as its own class
 * for the same reason AS3 does: the base class is what changes the slot.
 *
 ...
 *
 * AS3: sources/WIN63-202607011411-782849652/src/com/sulake/habbo/friendbar/view/tabs/NewFriendRequestTab.as
 */
export class NewFriendRequestTab extends NewFriendEntityTab
{
    // AS3: .../view/tabs/NewFriendRequestTab.as::REQUEST_WINDOW_RESOURCE
    protected static readonly REQUEST_WINDOW_RESOURCE: string = 'friend_request_tab_xml';

    // AS3: .../view/tabs/NewFriendRequestTab.as::BUTTON_ACCEPT
    protected static readonly REQUEST_BUTTON_ACCEPT: string = 'button_accept';

    /** **Name derived** from its value. */
    // AS3: .../view/tabs/NewFriendRequestTab.as::BUTTON_CLOSE
    protected static readonly REQUEST_BUTTON_CLOSE: string = 'button_close';

    // AS3: .../view/tabs/NewFriendRequestTab.as::REGION_REJECT
    protected static readonly REGION_REJECT: string = 'click_region_reject';

    // AS3: .../view/tabs/NewFriendRequestTab.as::REGION_REJECT_TEXT
    protected static readonly REGION_REJECT_TEXT: string = 'link_reject';

    // AS3: .../view/tabs/NewFriendRequestTab.as::DEFAULT_COLOR
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::REGION_REJECT_COLOR_EXPOSED
    private static readonly REGION_REJECT_COLOR_EXPOSED: number = 0xFFE66A;

    // AS3: .../view/tabs/NewFriendRequestTab.as::REGION_REJECT_COLOR_NORMAL
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::ICON_RESOURCE
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::_recycledInstance
    private static _recycledRequestTab: NewFriendRequestTab | null = null;

    /**
     * Reuses the single instance. The head is only redrawn when the request's figure
     * differs from the one already shown.
     */
    // AS3: .../view/tabs/NewFriendRequestTab.as::allocate()
    static allocateRequest(request: IFriendRequest): NewFriendRequestTab
    {
        const tab = NewFriendRequestTab._recycledRequestTab ?? new NewFriendRequestTab();

        tab._recycled = false;

        if(tab.friend !== null && tab.friend.figure !== request.figure)
        {
            const canvas = tab._window?.findChildByName(NewFriendEntityTab.CANVAS) as IBitmapWrapperWindow | null;
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::FriendRequestTab()
    constructor()
    {
        super();
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::recycle()
    override recycle(): void
    {
        ...
        NewFriendRequestTab._recycledRequestTab = this;
    }

    // AS3: .../view/tabs/NewFriendRequestTab.as::select()
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::deselect()
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::expose()
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::conceal()
    ...
    private applyRequestHoverStyle(): void
    {
        ...
        {
            this._window.color = this.exposed ? NewFriendRequestTab.REQUEST_HOVER_COLOR : NewFriendRequestTab.REQUEST_DEFAULT_COLOR;
        }
    }

    // AS3: .../view/tabs/NewFriendRequestTab.as::allocateRequestTabWindow()
    private allocateRequestTabWindow(): IWindowContainer | null
    {
        const window = Tab.windowing?.buildWidgetLayout(NewFriendRequestTab.REQUEST_WINDOW_RESOURCE) as IWindowContainer | null;

        if(window === null || window === undefined)
        {
            log.error(`allocateRequestTabWindow: layout "${NewFriendRequestTab.REQUEST_WINDOW_RESOURCE}" is not registered`);
        ...
        if(iconRegion !== null && iconRegion !== undefined)
        {
            ...
            if(bitmap !== null && bitmap !== undefined)
            {
                ...
                bitmap.bitmap = (Tab.assets?.getAssetByName(NewFriendRequestTab.PLUS_ICON_RESOURCE)?.content as ImageBitmap | null) ?? null;
                (iconRegion as unknown as IWindowContainer).addChild(bitmap as unknown as IWindow);
            }

            const iconList = window.findChildByName(NewFriendEntityTab.ICONS) as IItemListWindow | null;
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::releaseRequestTabWindow()
    private releaseRequestTabWindow(window: IWindowContainer): void
    {
        ...
        window.width = Tab.width;
        window.height = Tab.height;
        window.color = NewFriendRequestTab.REQUEST_DEFAULT_COLOR;
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::bubbleEventProc()
    private requestBubbleEventProc = (event: WindowEvent, window: IWindow): void =>
    {
        ...
        if(event.type === 'WME_CLICK')
        {
            switch(window.name)
            {
                case NewFriendRequestTab.REQUEST_BUTTON_ACCEPT:
                    Tab.data?.acceptFriendRequest(friendId);
                    break;

                case NewFriendRequestTab.REQUEST_BUTTON_CLOSE:
                    if(this.selected)
                    {
                ...
                case NewFriendRequestTab.REGION_REJECT:
        ...
        if(event.type === 'WME_OVER' || event.type === 'WME_OUT')
        {
            ...
            if(window.name === NewFriendRequestTab.REGION_REJECT)
            {
                const text = container.getChildByName(NewFriendRequestTab.REGION_REJECT_TEXT) as ITextWindow | null;

                if(text !== null)
                {
                    text.textColor = over ? NewFriendRequestTab.REGION_REJECT_COLOR_EXPOSED : NewFriendRequestTab.REGION_REJECT_COLOR_NORMAL;
                }
            }

            if(window.name === NewFriendEntityTab.BTN_PROFILE)
            {
                const icon = container.findChildByName(NewFriendEntityTab.ICON) as unknown as IIconWindow | null;

                if(icon !== null)
                {
                    icon.style = over ? NewFriendRequestTab.PROFILE_ICON_STYLE_OVER : NewFriendRequestTab.PROFILE_ICON_STYLE_OUT;
                }
            }

            if(window.name === NewFriendEntityTab.PROFILE)
            {
                const name = container.getChildByName(NewFriendEntityTab.NAME) as ITextWindow | null;
     ...
     */
    // AS3: .../view/tabs/NewFriendRequestTab.as::avatarImageReady()
    avatarImageReady(request: IFriendRequest, _face: ImageBitmap): void
    {
        ...
        const canvas = this._window?.findChildByName(NewFriendEntityTab.CANVAS) as IBitmapWrapperWindow | null;
    ...
    // AS3: .../view/tabs/NewFriendRequestTab.as::dispose()
