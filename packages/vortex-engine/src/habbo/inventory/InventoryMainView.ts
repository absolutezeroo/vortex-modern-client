 ...
 */
const log = Logger.getLogger('habbo.inventory.InventoryMainView');
...
export class InventoryMainView 
{
    ...
    /**
     * TS-only: adds the Vortex fishing tab.
     *
     * The dump's `inventory_xml` carries every other tab button, and it is regenerated by
     * tools/build-window-assets.mjs — so a fishing entry there would be both a divergence from the
     * dump and lost on the next asset build. The button is authored separately
     * (`vortex_fishing_tab_xml`) and added here instead, the same way the furni editor adds its own
     * button to the infostand.
     *
     * Skipped silently when the fishing component is absent: `getCategoryWindowContainer('fishing')`
     * would then answer null and the tab would open onto nothing.
     *
     * See docs/vortex-original/fishing.md 2.6.
     */
    // TS-only: Vortex-only fishing system — no AS3 counterpart.
    private addFishingTab(tabs: ITabContextWindow): void
    {
        // Registered here rather than trusted to have been registered at init: fishing arrives on an
        // optional DI dependency, so whether it had resolved by then was a race the tab silently
        // lost.
        this._habboInventory.ensureFishingModel();

        if(this._habboInventory.getCategoryWindowContainer('fishing') === null)
        {
            log.debug('No fishing model registered; the fishing tab is not added.');

            return;
        }

        const tab = this._habboInventory.windowManager?.buildWidgetLayout('vortex_fishing_tab_xml') as ITabButtonWindow | null;

        if(tab === null || tab === undefined)
        {
            log.warn('vortex_fishing_tab_xml is not registered; the fishing tab cannot be added.');
