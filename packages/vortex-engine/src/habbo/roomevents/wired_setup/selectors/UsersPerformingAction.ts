...
/**
 * UsersPerformingAction — the "select users performing a given action" wired selector: an action
 ...
 *
 * Name derived: the AS3 class is obfuscated as `_SafeCls_4286`; the name follows the code it returns
 * (SelectorCodes.USERS_PERFORMING_ACTION). Body is byte-identical to the condition PerformingAction
 * and the trigger UserPerformsAction (AS3 duplicates it three times).
 *
 * AS3: sources/WIN63-202607011411-782849652/src/com/sulake/habbo/roomevents/wired_setup/selectors/_SafeCls_4286.as
 */
export class UsersPerformingAction extends DefaultSelectorType
{
    // AS3: _SafeCls_4286.as::SIGN_ACTION_CODE (decompiler inlined the literal 10; usage restored)
    private static readonly SIGN_ACTION_CODE: number = 10;

    // AS3: _SafeCls_4286.as::DANCE_ACTION_CODE (decompiler inlined the literal 11; usage restored)
    private static readonly DANCE_ACTION_CODE: number = 11;

    // AS3: _SafeCls_4286.as::_actionDropdown
    private _actionDropdown!: DropdownPreset;

    // AS3: _SafeCls_4286.as::_SafeStr_7030 (name derived: the sign-filter checkbox group)
    private _signCheckboxGroup!: CheckboxGroupPreset;

    // AS3: _SafeCls_4286.as::_SafeStr_7159 (name derived: the dance-filter checkbox group)
    private _danceCheckboxGroup!: CheckboxGroupPreset;

    // AS3: _SafeCls_4286.as::_signDropdown
    private _signDropdown!: DropdownPreset;

    // AS3: _SafeCls_4286.as::_danceDropdown
    private _danceDropdown!: DropdownPreset;

    // AS3: _SafeCls_4286.as::_SafeStr_6939 (name derived: the sign-selection section)
    private _signSection!: SectionPreset;

    // AS3: _SafeCls_4286.as::_SafeStr_7143 (name derived: the dance-selection section)
    private _danceSection!: SectionPreset;

    // AS3: _SafeCls_4286.as::getActionByCode()
    ...
    // AS3: _SafeCls_4286.as::readStringParamFromForm()
    ...
    // AS3: _SafeCls_4286.as::readIntParamsFromForm()
    ...
    // AS3: _SafeCls_4286.as::get inputMode()
    override get inputMode(): number
    {
        return DefaultElement.INPUTS_TYPE_UI_BUILDER;
    }

    // AS3: _SafeCls_4286.as::buildInputs()
    ...
    // AS3: _SafeCls_4286.as::onActionSelected() — the dropdown option arg is unused (AS3 ignores it).
    private onActionSelected(): void
    {
        this.updateExtraSections();
    }

    // AS3: _SafeCls_4286.as::updateExtraSections()
    private updateExtraSections(stringParam: string = ''): void
    {
        ...
        const isSign = action.code === UsersPerformingAction.SIGN_ACTION_CODE;
        const isDance = action.code === UsersPerformingAction.DANCE_ACTION_CODE;
    ...
    // AS3: _SafeCls_4286.as::getSelectedAction()
    private getSelectedAction(): WiredUserAction | null
    {
        return UsersPerformingAction.getActionByCode(this._actionDropdown.selectedId);
    }

    // AS3: _SafeCls_4286.as::getSelectedExtraCode()
    private getSelectedExtraCode(actionCode: number): number
    {
        if(actionCode === UsersPerformingAction.SIGN_ACTION_CODE)
        {
            return this._signCheckboxGroup.optionById(0).selected ? this._signDropdown.selectedId : -1;
        }

        if(actionCode === UsersPerformingAction.DANCE_ACTION_CODE)
        {
    ...
    // AS3: _SafeCls_4286.as::buildActionOptions()
    ...
    // AS3: _SafeCls_4286.as::buildSignOptions()
    ...
    // AS3: _SafeCls_4286.as::buildDanceOptions()
