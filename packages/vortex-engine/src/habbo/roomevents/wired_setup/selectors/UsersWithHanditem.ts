...
/**
 * UsersWithHanditem — the "select users holding a given hand item" wired selector: a hand-item
 * dropdown (a default set of codes, extended on demand) plus a "capture" button that reads the hand
 * item the player is currently holding. The chosen hand-item code is stored in intParams[0].
 *
 * Name derived: the AS3 class is obfuscated as `_SafeCls_4047`; the name follows the code it returns
 * (SelectorCodes.USERS_WITH_HANDITEM). Body is byte-identical to the condition ActorHasHanditem.
 *
 * AS3: sources/WIN63-202607011411-782849652/src/com/sulake/habbo/roomevents/wired_setup/selectors/_SafeCls_4047.as
 */
export class UsersWithHanditem extends DefaultSelectorType
{
    // AS3: _SafeCls_4047.as::DEFAULT_CODES
    private static readonly DEFAULT_CODES: number[] = [0, 2, 5, 7, 8, 9, 10, 27];

    // AS3: _SafeCls_4047.as::_handItemDropdown
    private _handItemDropdown!: DropdownPreset;

    // AS3: _SafeCls_4047.as::_SafeStr_8793 (name derived: the capture button)
    private _captureButton!: ButtonPreset;

    // AS3: _SafeCls_4047.as::_SafeStr_5343 (name derived: the current dropdown options)
    private _options!: ExpandableDropdownOption[];

    // AS3: _SafeCls_4047.as::get code()
    override get code(): number
    {
        return SelectorCodes.USERS_WITH_HANDITEM;
    }

    // AS3: _SafeCls_4047.as::get inputMode()
    override get inputMode(): number
    {
        return DefaultElement.INPUTS_TYPE_UI_BUILDER;
    }

    // AS3: _SafeCls_4047.as::buildInputs()
    override buildInputs(presetManager: PresetManager, _wiredStyle: WiredStyle, builder: WiredUIBuilder): void
    {
        this._options = this.createOptions(UsersWithHanditem.DEFAULT_CODES);
    ...
    // AS3: _SafeCls_4047.as::readIntParamsFromForm()
    ...
    // AS3: _SafeCls_4047.as::ensureOptionExists()
    ...
    // AS3: _SafeCls_4047.as::setSelectedHandItemByCode()
    ...
    // AS3: _SafeCls_4047.as::captureHanditem()
