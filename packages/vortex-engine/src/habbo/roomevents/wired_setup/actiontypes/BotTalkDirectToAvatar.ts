...
/**
 * BotTalkDirectToAvatar — the "make a bot whisper / talk directly to an avatar" wired action: a
 * bot-name text input, a chat-message text area and a whisper/talk toggle, stored as the string param
 * "name\tmessage" plus intParams [operation].
 *
 * Name derived: the AS3 class is obfuscated as `_SafeCls_4292`; the name follows the code it returns
 * (ActionTypeCodes.BOT_TALK_DIRECT_TO_AVTR — "avtr" expands to "avatar").
 *
 * AS3: sources/WIN63-202607011411-782849652/src/com/sulake/habbo/roomevents/wired_setup/actiontypes/_SafeCls_4292.as
 */
export class BotTalkDirectToAvatar extends DefaultBotActionType
{
    // AS3: _SafeCls_4292.as::STRING_PARAM_DELIMITER (declared but unused; AS3 uses the literal '\t' inline)
    private static readonly STRING_PARAM_DELIMITER: string = '\t';

    // AS3: _SafeCls_4292.as::_botName
    private _botName!: TextInputPreset;

    // AS3: _SafeCls_4292.as::_chatMessage
    private _chatMessage!: TextAreaPreset;

    // AS3: _SafeCls_4292.as::_SafeStr_7368 (name derived)
    private _operation!: RadioGroupPreset;

    // AS3: _SafeCls_4292.as::get code()
    override get code(): number
    {
        return ActionTypeCodes.BOT_TALK_DIRECT_TO_AVTR;
    }

    // AS3: _SafeCls_4292.as::readStringParamFromForm()
    override readStringParamFromForm(): string
    {
        return this._botName.text + '\t' + this._chatMessage.text;
    }

    // AS3: _SafeCls_4292.as::onEditStart()
    ...
    // AS3: _SafeCls_4292.as::readIntParamsFromForm()
    override readIntParamsFromForm(): number[]
    {
        return [this._operation.selected];
    }

    // AS3: _SafeCls_4292.as::get inputMode()
    override get inputMode(): number
    {
        return DefaultElement.INPUTS_TYPE_UI_BUILDER;
    }

    // AS3: _SafeCls_4292.as::buildInputs()
    override buildInputs(presetManager: PresetManager, _wiredStyle: WiredStyle, builder: WiredUIBuilder): void
    {
        ...
        this._operation = presetManager.createRadioGroup([
            new RadioButtonParam(1, this.l('whisper')),
