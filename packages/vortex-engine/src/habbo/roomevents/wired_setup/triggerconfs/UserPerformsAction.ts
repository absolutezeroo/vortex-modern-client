...
/**
 * UserPerformsAction — the "a user performs a given action" wired trigger configuration: an action
 * dropdown (wave/blow/.../sign/dance), plus a sign-index and a dance-index dropdown revealed (each
 * behind its own checkbox) only when the selected action is sign (#10) or dance (#11). The action
 * code goes to intParams[0]; the extra sign/dance index is encoded into stringParam.
 *
 * Name derived: the AS3 class is obfuscated as `_SafeCls_4035` and its code constant is likewise a
 * value-named `TriggerConfCodes.TRIGGER_CODE_16` — no real name exists in any source tree. The name is
 * derived from behaviour (this is the trigger twin of the condition PerformingAction / selector
 * UsersPerformingAction; AS3 duplicates the body byte-identically across all three).
 *
 * AS3: sources/WIN63-202607011411-782849652/src/com/sulake/habbo/roomevents/wired_setup/triggerconfs/_SafeCls_4035.as
 */
export class UserPerformsAction extends DefaultTriggerConf
{
    // AS3: _SafeCls_4035.as::SIGN_ACTION_CODE (decompiler inlined the literal 10; usage restored)
    private static readonly SIGN_ACTION_CODE: number = 10;

    // AS3: _SafeCls_4035.as::DANCE_ACTION_CODE (decompiler inlined the literal 11; usage restored)
    private static readonly DANCE_ACTION_CODE: number = 11;

    // AS3: _SafeCls_4035.as::_actionDropdown
    private _actionDropdown!: DropdownPreset;

    // AS3: _SafeCls_4035.as::_SafeStr_7030 (name derived: the sign-filter checkbox group)
    private _signCheckboxGroup!: CheckboxGroupPreset;

    // AS3: _SafeCls_4035.as::_SafeStr_7159 (name derived: the dance-filter checkbox group)
    private _danceCheckboxGroup!: CheckboxGroupPreset;

    // AS3: _SafeCls_4035.as::_signDropdown
    private _signDropdown!: DropdownPreset;

    // AS3: _SafeCls_4035.as::_danceDropdown
    private _danceDropdown!: DropdownPreset;

    // AS3: _SafeCls_4035.as::_SafeStr_6939 (name derived: the sign-selection section)
    private _signSection!: SectionPreset;

    // AS3: _SafeCls_4035.as::_SafeStr_7143 (name derived: the dance-selection section)
    private _danceSection!: SectionPreset;

    // AS3: _SafeCls_4035.as::getActionByCode()
    ...
    // AS3: _SafeCls_4035.as::get code()
    override get code(): number
    {
        return TriggerConfCodes.TRIGGER_CODE_16;
    }
    ...
    // AS3: _SafeCls_4035.as::readStringParamFromForm()
    ...
    // AS3: _SafeCls_4035.as::readIntParamsFromForm()
    ...
    // AS3: _SafeCls_4035.as::get inputMode()
    override get inputMode(): number
    {
        return DefaultElement.INPUTS_TYPE_UI_BUILDER;
    }

    // AS3: _SafeCls_4035.as::buildInputs()
    ...
    // AS3: _SafeCls_4035.as::onActionSelected() — the dropdown option arg is unused (AS3 ignores it).
    private onActionSelected(): void
    {
        this.updateExtraSections();
    }

    // AS3: _SafeCls_4035.as::updateExtraSections()
    private updateExtraSections(stringParam: string = ''): void
    {
        ...
        const isSign = action.code === UserPerformsAction.SIGN_ACTION_CODE;
        const isDance = action.code === UserPerformsAction.DANCE_ACTION_CODE;
    ...
    // AS3: _SafeCls_4035.as::getSelectedAction()
    private getSelectedAction(): WiredUserAction | null
    {
        return UserPerformsAction.getActionByCode(this._actionDropdown.selectedId);
    }

    // AS3: _SafeCls_4035.as::getSelectedExtraCode()
    private getSelectedExtraCode(actionCode: number): number
    {
        if(actionCode === UserPerformsAction.SIGN_ACTION_CODE)
        {
            return this._signCheckboxGroup.optionById(0).selected ? this._signDropdown.selectedId : -1;
        }

        if(actionCode === UserPerformsAction.DANCE_ACTION_CODE)
        {
    ...
    // AS3: _SafeCls_4035.as::buildActionOptions()
    ...
    // AS3: _SafeCls_4035.as::buildSignOptions()
    ...
    // AS3: _SafeCls_4035.as::buildDanceOptions()
