...
/**
 * FurniNamePlaceholder — the "furni name placeholder" wired addon: a placeholder-name section and a
 * placeholder-type section (furni). Stored as the string param (name, optionally "name\tdelimiter"
 * when showing multiple) plus intParams [showMultiple].
 *
 * Name derived: the AS3 class is obfuscated as `_SafeCls_4442`; the name follows the code it returns
 * (AddonCodes.FURNI_NAME_PLACEHOLDER).
 *
 * AS3: sources/WIN63-202607011411-782849652/src/com/sulake/habbo/roomevents/wired_setup/addons/_SafeCls_4442.as
 */
export class FurniNamePlaceholder extends DefaultAddonType
{
    // AS3: _SafeCls_4442.as::_nameSection
    private _nameSection!: PlaceholderNameSection;

    // AS3: _SafeCls_4442.as::_typeSection
    private _typeSection!: PlaceholderTypeSection;

    // AS3: _SafeCls_4442.as::get code()
    override get code(): number
    {
        return AddonCodes.FURNI_NAME_PLACEHOLDER;
    }

    // AS3: _SafeCls_4442.as::get inputMode()
    override get inputMode(): number
    {
        return DefaultElement.INPUTS_TYPE_UI_BUILDER;
    }

    // AS3: _SafeCls_4442.as::buildInputs()
    override buildInputs(presetManager: PresetManager, _wiredStyle: WiredStyle, builder: WiredUIBuilder): void
    {
        this._nameSection = presetManager.createPlaceholderNameSection(this.l('texts.placeholder_name'), '$');
        this._typeSection = presetManager.createPlaceholderTypeSection('furni');

        builder.addElements(this._nameSection, this._typeSection);
    }

    // AS3: _SafeCls_4442.as::onEditStart()
    ...
    // AS3: _SafeCls_4442.as::readIntParamsFromForm()
    override readIntParamsFromForm(): number[]
    {
        return [this._typeSection.isShowMultiple ? 1 : 0];
    }

    // AS3: _SafeCls_4442.as::readStringParamFromForm()
