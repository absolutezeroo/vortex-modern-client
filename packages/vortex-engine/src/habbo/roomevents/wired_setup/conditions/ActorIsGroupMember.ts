...
/**
 * ActorIsGroupMember — the "actor belongs to a group" wired condition: a radio group choosing between
 * "any group" and "a specific group", the latter revealing a group dropdown populated from the
 * player's guild memberships (requested on edit, throttled, and refreshed by onGuildMemberships). The
 * chosen group id is stored in stringParam (empty for "any group").
 *
 * Name derived: the AS3 class is obfuscated as `_SafeCls_4103`; the name follows the code it returns
 * (ConditionCodes.ACTOR_IS_GROUP_MEMBER). Body is byte-identical to the selector UsersInGroup.
 *
 * AS3: sources/WIN63-202607011411-782849652/src/com/sulake/habbo/roomevents/wired_setup/conditions/_SafeCls_4103.as
 */
export class ActorIsGroupMember extends DefaultConditionType
{
    // AS3: _SafeCls_4103.as::REQUEST_TIMEOUT (seconds between membership requests)
    private static readonly REQUEST_TIMEOUT: number = 5;

    // AS3: _SafeCls_4103.as::_SafeStr_7036 (name derived: the cached guild memberships)
    private _guilds: GuildMembership[] | null = null;

    // AS3: _SafeCls_4103.as::_SafeStr_4935 (name derived: the group id to preselect)
    private _selectedGroupId: number = -1;

    // AS3: _SafeCls_4103.as::_SafeStr_9417 (name derived: the last membership request time, ms)
    private _lastRequestTime: number = 0;

    // AS3: _SafeCls_4103.as::_SafeStr_7031 (name derived: the any-group / specific-group radio)
    private _typeRadio!: RadioGroupPreset;

    // AS3: _SafeCls_4103.as::_groupDropdown
    private _groupDropdown!: DropdownPreset;

    // AS3: _SafeCls_4103.as::get code()
    override get code(): number
    {
        return ConditionCodes.ACTOR_IS_GROUP_MEMBER;
    }

    // AS3: _SafeCls_4103.as::get negativeCode()
    override get negativeCode(): number
    {
        return ConditionCodes.NOT_ACTOR_IS_GROUP_MEMBER;
    }

    // AS3: _SafeCls_4103.as::get inputMode()
    override get inputMode(): number
    {
        return DefaultElement.INPUTS_TYPE_UI_BUILDER;
    }

    // AS3: _SafeCls_4103.as::buildInputs()
    ...
    // AS3: _SafeCls_4103.as::readStringParamFromForm()
    ...
    // AS3: _SafeCls_4103.as::onEditStart()
    ...
    // AS3: _SafeCls_4103.as::maybeGetGuildMemberships()
    private maybeGetGuildMemberships(): void
    {
        const now = Date.now();

        if(now > this._lastRequestTime + 1000 * ActorIsGroupMember.REQUEST_TIMEOUT)
        {
    ...
    // AS3: _SafeCls_4103.as::initGuilds()
    ...
    // AS3: _SafeCls_4103.as::onGuildMemberships() — receives a GuildMembershipsMessageEvent (typed
