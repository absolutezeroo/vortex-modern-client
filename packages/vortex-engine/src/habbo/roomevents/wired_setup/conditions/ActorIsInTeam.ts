...
/**
 * ActorIsInTeam — the "the actor belongs to a team" wired condition: an any/1..4 team selector (two
 * columns), stored as intParams [team]. Exposes the negation (NOT_ACTOR_IS_IN_TEAM).
 *
 * Name derived: the AS3 class is obfuscated as `_SafeCls_4112`; the name follows the code it returns
 * (ConditionCodes.ACTOR_IS_IN_TEAM).
 *
 * AS3: sources/WIN63-202607011411-782849652/src/com/sulake/habbo/roomevents/wired_setup/conditions/_SafeCls_4112.as
 */
export class ActorIsInTeam extends DefaultConditionType
{
    // AS3: _SafeCls_4112.as::_team
    private _team!: RadioGroupPreset;

    // AS3: _SafeCls_4112.as::get code()
    override get code(): number
    {
        return ConditionCodes.ACTOR_IS_IN_TEAM;
    }

    // AS3: _SafeCls_4112.as::get negativeCode()
    override get negativeCode(): number
    {
        return ConditionCodes.NOT_ACTOR_IS_IN_TEAM;
    }

    // AS3: _SafeCls_4112.as::get inputMode()
    override get inputMode(): number
    {
        return DefaultElement.INPUTS_TYPE_UI_BUILDER;
    }

    // AS3: _SafeCls_4112.as::buildInputs()
