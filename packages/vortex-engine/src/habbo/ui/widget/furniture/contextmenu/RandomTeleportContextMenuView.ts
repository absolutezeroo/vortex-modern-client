...
const log = Logger.getLogger('habbo.ui.widget.furniture.contextmenu.RandomTeleportContextMenuView');

/**
 * RandomTeleportContextMenuView
 *
 * The context bubble over a random teleport: its own layout, a name row, and a single Use button that
 ...
 *
 * AS3: sources/WIN63-202607011411-782849652/src/com/sulake/habbo/ui/widget/furniture/contextmenu/RandomTeleportContextMenuView.as
 */
export class RandomTeleportContextMenuView extends FurnitureContextInfoView
{
    // AS3: .../contextmenu/RandomTeleportContextMenuView.as::RandomTeleportContextMenuView()
    constructor(widget: IContextMenuParentWidget)
    {
        super(widget);
    }

    // AS3: .../contextmenu/RandomTeleportContextMenuView.as::_SafeStr_8829
    private _objectCategory: number = 0;

    // AS3: .../contextmenu/RandomTeleportContextMenuView.as::set objectCategory()
    public set objectCategory(value: number)
    {
        this._objectCategory = value;
    }

    // AS3: .../contextmenu/RandomTeleportContextMenuView.as::updateWindow()
    protected override updateWindow(): void
    {
        ...
        {
            const asset = this._widget.assets.getAssetByName('random_teleport_menu') as XmlAsset | null;

            if(asset === null || asset === undefined)
            {
                log.warn('Missing layout "random_teleport_menu" - the bubble cannot open');
        ...
        if(furniName !== null)
        {
            furniName.caption = '${furni.random_teleport.name}';
        }
    ...
    // AS3: .../contextmenu/RandomTeleportContextMenuView.as::buttonEventProc()
