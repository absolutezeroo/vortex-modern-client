/**
 * MyIssuesView — the issue browser's "my issues" tab: the bundles this moderator has picked.
 *
 * @see sources/WIN63-202607011411-782849652/src/com/sulake/habbo/moderation/MyIssuesView.as
 *
 * The only tab with a "release all" button; the other two are the same shape without it. All three
 * start hidden — `IssueBrowser.selectView()` is what shows one.
 */
...
export class MyIssuesView implements IIssueBrowserView
{
    ...
    // AS3: MyIssuesView.as::set visible()
    ...
    // AS3: MyIssuesView.as::update()
    public update(): void
    {
        this._listView.update(this._issueManager.getBundles(IssueManager.BUNDLE_MY));
    }

    // AS3: MyIssuesView.as::onReturnAll()
    private onReturnAll = (): void =>
    {
