...
export class RoomSettingsCtrl
{
    ...
    private populateForm(): void
    {
        ...
        if(allowPetsCb) data.allowPets ? allowPetsCb.select() : allowPetsCb.unselect();

        const allowFoodCb = this._window.findChildByName('allow_foodconsume_checkbox') as unknown as ICheckable | null;
        if(allowFoodCb) data.allowFoodConsume ? allowFoodCb.select() : allowFoodCb.unselect();

        const allowWalkCb = this._window.findChildByName('allow_walk_through_checkbox') as unknown as ICheckable | null;
        if(allowWalkCb) data.allowWalkThrough ? allowWalkCb.select() : allowWalkCb.unselect();

        const allowDynCatsCb = this._window.findChildByName('allow_dyncats_checkbox') as unknown as ICheckable | null;
        if(allowDynCatsCb) data.allowNavigatorDynamicCats ? allowDynCatsCb.select() : allowDynCatsCb.unselect();
        ...
        if(hideWallsCb) data.hideWalls ? hideWallsCb.select() : hideWallsCb.unselect();
